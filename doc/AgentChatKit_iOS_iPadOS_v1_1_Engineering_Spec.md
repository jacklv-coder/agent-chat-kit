...
> 规格版本：1.1-draft  
> 核对日期：2026-08-10  
> 修订标识：2026-08-10-r2（移除 ChatLayout 强制依赖，改为 UIKit 原生布局与自研滚动协调）  
> 仓库建议：`agent-chat-kit`  
> Swift Package：`AgentChatKit`  
> 首发平台：iOS 17+、iPadOS 17+  
> 技术路线：UIKit-first、UICollectionView、UICollectionViewCompositionalLayout、Swift Concurrency、Swift Package Manager  
> 许可证：Apache-2.0  
...
8. 不允许在流式过程中调用 `reloadData()`，也不允许每个 token 重建完整 Diffable Snapshot。
9. v1.0 不允许把 ChatLayout 或其他第三方聊天布局库作为强制依赖；会话布局基于 UIKit 原生 `UICollectionViewCompositionalLayout`，滚动锚点与更新调度由 AgentChatKit 自己实现。
10. 不允许依赖 `IndexPath` 作为跨更新、跨分页或跨旋转的稳定标识。
11. 不允许在 Core、Runtime SPI、Reducer 中导入 UIKit。
12. v1.0 不实现 AppKit；但公共模型和 Runtime SPI 必须保持平台无关，为后续 macOS 复用。
13. 所有 public API 必须有 DocC 注释；所有 public 类型必须显式评估 `Sendable`。
14. 每次改动公共 API，都必须同步更新示例、测试、README 和 Changelog。
15. 完成标准以本文“v1.0 验收清单”为准，而不是以“页面看起来能运行”为准。
16. 如规范中的示例代码存在轻微 Swift 语法问题，可以做最小修正；不得改变架构边界。
17. 最终交付前必须执行完整构建、单元测试、UI 测试、性能场景、滚动稳定性测试和无障碍检查，并输出结果。
...
v1.0 不引入第三方 `UICollectionViewLayout` 或完整聊天 UI 框架。测试依赖可包含：

- `swift-snapshot-testing`，仅用于测试；
- 其他依赖必须是 test-only，并在 `NOTICE` 与文档中说明用途。

### 4.4 布局依赖决策

AgentChatKit 的时间线以“全宽 Agent Block”为主，用户消息的 trailing 对齐在 Cell 内完成，不需要传统 IM 的左右气泡布局系统。因此 v1.0 正式决定：

```text
UICollectionView
+ UICollectionViewCompositionalLayout
+ UICollectionViewDiffableDataSource
+ AgentUpdateScheduler
+ AgentScrollCoordinator
+ AgentLayoutAnchor
+ AgentItemSizeCache
```

不把 ChatLayout 作为强制依赖，原因如下：

- 降低开源基础组件的依赖面与长期兼容风险；
- 掌握流式高度变化、历史 prepend、键盘变化和 iPad resize 的滚动控制权；
- 避免第三方布局类型泄漏到 public API；
- Agent 时间线结构比传统 IM 简单，系统布局足以作为首版基线；
- 即使未来需要自研 `AgentTimelineLayout`，也能保持为内部实现，不影响接入方代码。

只有当原生 Compositional Layout 在本文规定的压力场景中无法通过验收，才允许新增内部 `AgentTimelineLayout`。新增前必须提交基准数据、失败复现和替换设计；不得直接重新引入强制第三方布局依赖。
...
- 禁止让具体 Layout 类型泄漏到 public API；
- 禁止把 ChatLayout 或其他聊天 UI 框架设为必选依赖；
- 禁止依赖单例屏幕尺寸或单例 Session；
- 禁止在 Cell 内直接发起 Runtime 命令；
- 禁止把 `contentOffset` 的偶然数值当作唯一滚动状态；
- 禁止通过全量 reload 掩盖自适应高度或锚点恢复问题。
...
│   │   ├── DataSource/
│   │   ├── Layout/
│   │   │   ├── AgentConversationLayoutFactory.swift
│   │   │   ├── AgentTimelineLayoutConfiguration.swift
│   │   │   └── AgentItemSizeCache.swift
│   │   ├── Scrolling/
│   │   │   ├── AgentScrollCoordinator.swift
│   │   │   ├── AgentLayoutAnchor.swift
│   │   │   └── AgentUpdateScheduler.swift
│   │   ├── Rendering/
│   │   ├── Cells/
│   │   ├── Blocks/
│   │   ├── Markdown/
...
布局实现、滚动协调器、更新调度器和 Diffable Data Source 均为内部细节，不出现在 public initializer 中。
...
├── UICollectionView
│   └── UICollectionViewCompositionalLayout（内部实现）
├── JumpToLatestButton
├── ConnectionBanner
└── ComposerContainer
    └── AgentComposerView
...
Assistant Header、时间、状态等使用 Supplementary View，不创建伪 Block。任何 UI 更新都通过稳定 ID 定位，不长期保存 `IndexPath`。

### 9.4 原生布局实现

v1.0 使用系统 `UICollectionViewCompositionalLayout`，不依赖第三方聊天布局库。
...
- Cell 通过 Auto Layout 和 self-sizing 得到最终高度；
- 使用估算高度，不把最终高度硬编码进布局；
- User Block 的靠右和最大宽度在 Cell 内容约束中实现；
- Assistant、Tool、Command、Diff、Approval 默认占可用内容列全宽；
- iPad 的最大内容宽度由 section inset 计算，不嵌套第二个垂直 Scroll View；
- 旋转、Split View、Stage Manager resize 时重新计算布局环境；
- Layout 工厂、配置和任何未来的 `AgentTimelineLayout` 都保持 internal；
- public API 只暴露语义配置，例如最大内容宽度、间距和滚动策略。

如果系统布局无法通过本文的滚动与性能验收，可以在不改变 public API 的前提下替换为内部 `AgentTimelineLayout`。不得让宿主依赖具体 Layout 类型。

### 9.5 Diffable Data Source 与更新分类
...
UIKit 层必须把 Patch 分类为三种内部更新：

```swift
internal enum AgentCollectionUpdateKind {
    case structural
    case sizeAffecting
    case contentOnly
}
...
- `structural`
  ...
  - 新增/删除/移动 Block；
  - 历史分页 prepend；
  - 通过稳定 ID 构造并 apply Diffable Snapshot。
- `sizeAffecting`
  - Markdown 文本增长；
  - Command Output 追加；
  - Tool、Diff、Approval 展开收起；
  - Dynamic Type、主题或宽度变化；
  - 必须在 `AgentScrollCoordinator` 的锚点事务中执行；
  - 可使用 `snapshot.reloadItems`、`reconfigureItems` 配合目标布局失效，或其他经过测试的局部方式；
  - 不得假设单独调用 `reconfigureItems` 一定会触发正确的高度重算。
- `contentOnly`
  - 不改变高度的图标、状态、进度或辅助文字变化；
  - 优先使用 `reconfigureItems([blockID])`。
...
- 禁止流式阶段 `reloadData()`；
- 禁止通过随机延时或多次 `scrollToItem` 修复跳动；
- 更新完成后，Data Source 中的 ID 顺序必须与 Store Snapshot 一致。

### 9.6 AgentUpdateScheduler

Store 可以高频更新，UIKit 层必须通过 `AgentUpdateScheduler` 合并 UI 工作：

```swift
@MainActor
internal final class AgentUpdateScheduler {
...
- 连续文字 Delta 默认在 33～80ms 的可配置窗口内合并；
- 同一显示帧最多执行一次布局相关批次；
- 同一 Block 的多个 Delta 合并为最新 revision；
- `approvalRequested`、`runFailed`、`runCompleted`、`turnCompleted` 等关键状态可以立即 flush；
- Background 时暂停动画，只保存最新状态；
- Foreground 后用最新 Store Snapshot 做一次校准；
- Scheduler 不持有 Runtime Connection；
- Scheduler 取消后不得再触发已销毁 ViewController 的 UI 更新。

可以使用 `CADisplayLink`、MainActor Task coalescer 或等价机制，但必须通过性能测试证明不会按 token 刷新布局。

### 9.7 AgentScrollCoordinator 与语义滚动状态

不得只用“当前 contentOffset”判断聊天状态。必须维护语义模式：

```swift
@MainActor
internal enum AgentScrollMode {
...
```swift
internal struct AgentLayoutAnchor: Hashable {
...
`AgentScrollCoordinator` 至少负责：

- 判断用户是否正在跟随最新内容；
- 记录和恢复阅读锚点；
- 历史 prepend 后保持原可见 Block 的视口位置；
- 高度变化时补偿锚点上方的布局增量；
- 键盘、旋转和 iPad resize 后恢复语义位置；
- 管理 Jump to Latest 的显示、未读数量和动作；
- 避免程序化滚动与用户拖拽互相抢夺。

锚点恢复事务必须遵循：

```text
1. 更新前选择稳定 Block 作为 anchor
2. 记录该 Block 的 top/bottom 与 viewport 的相对偏移
3. 执行 Snapshot / 局部刷新
4. collectionView.layoutIfNeeded()
5. 重新查找同一 Block 的 layout frame
6. 计算 frame 差值并补偿 contentOffset
7. 校验该 Block 仍存在；若不存在，按邻近稳定 ID 回退
```

滚动规则：

1. 用户距离底部不超过默认 80pt，且未主动向上拖动时，视为 `followingLatest`；
2. 用户主动向上拖动后进入 `readingHistory`；
3. `readingHistory` 时新内容不得强制拉回底部；
4. `readingHistory` 收到新内容时显示“回到最新”按钮和可配置未读计数；
5. 点击按钮滚到最新并恢复 `followingLatest`；
6. prepend 历史时保持更新前第一个合适可见 Block 的 ID 与相对偏移；
7. Block 展开、收起、Markdown 增长和 Command Output 追加时保持阅读锚点；
8. 变化发生在锚点下方时，不补偿当前视口；变化发生在锚点上方时，按 frame 差值补偿；
9. 键盘出现与消失时保持当前语义位置；
10. 旋转和窗口尺寸变化时按 Block ID 恢复位置，而不是恢复旧像素坐标；
11. 用户正在拖拽或减速时，不执行会抢夺手势的自动滚动；
12. 目标 Block 被删除时，优先选择同 Turn 的最近 Block，再选择相邻 Turn；
13. 不依赖 IndexPath 作为长期锚点；
14. 不使用连续多次 `scrollToItem` 作为锚点恢复方案。

### 9.8 自适应高度与 AgentItemSizeCache

Cell 高度以 UIKit self-sizing 为事实来源，缓存只用于减少重复测量，不得覆盖真实布局结果。

至少实现：

```swift
internal struct AgentItemSizeCacheKey: Hashable {
    let blockID: AgentBlockID
    let revision: Int
    let widthBucket: Int
...
- Markdown、Diff、Command Output 等高成本内容可以缓存测量结果；
- 宽度、Theme、Dynamic Type、Block revision 变化必须失效；
- Cache 有容量上限；
- Memory Warning 清理非必要项；
- 不缓存 `IndexPath`；
- 不缓存 UIView、Cell 或 Auto Layout 约束对象；
- 估算高度错误不得导致内容截断；
- 流式 Block 在完成前使用分段 revision，避免每个 token 创建永久缓存项。
...
- Theme 变化；
- Dynamic Type 变化；
- 旧 revision 的异步结果不得覆盖新内容；
- Size Cache 与当前宽度、revision 一致。
...
`performPrimaryAction()` 分别承接硬件键盘的聚焦及发送/停止操作。为保持旧有 conformer
的源码兼容，SDK 为这三个新增要求提供保守的默认实现；完整的自定义 Composer 应覆盖它们。
...
- 不按 token `reloadData()`；
- 不按 token 完整 apply Snapshot；
- `AgentUpdateScheduler` 合并高频 Patch，同一显示帧最多一次布局相关更新；
- `AgentScrollCoordinator` 使用稳定 Block ID 恢复锚点；
- Markdown 后台解析；
...
- Composer 不跳动；
- resize 结束并稳定布局后，锚点漂移目标不超过 4pt。

#### 场景 F：历史 Prepend 与混合高度变化

- 连续 prepend 20 页历史；
- 每页 50～100 Blocks；
- 同时存在 Markdown 增长、Tool 展开和图片尺寸回填；
- 原可见锚点在布局稳定后的漂移不超过 2pt；
- 不使用 `reloadData()`；
- 不依赖 IndexPath 恢复位置。
...
- Cell reuse；
- Renderer fallback；
- `structural` / `sizeAffecting` / `contentOnly` 更新分类；
- Reconfigure；
- Insert / Delete；
- 展开收起；
- Markdown 持续增长；
...
- ViewController；
- `UICollectionViewCompositionalLayout` Layout Factory；
...
- 无第三方聊天布局生产依赖；
- 插入、更新、删除均按稳定 ID 完成；
- 流式不调用 `reloadData()`，不按 token apply 完整 Snapshot；
- Markdown 与 Command Output 高度持续变化时锚点稳定；
- 历史 prepend 后布局稳定时锚点漂移不超过 2pt；
- 用户阅读历史时不会被新内容拉回底部；
- iPhone 与 iPad 长列表可流畅滚动。
...
- [ ] Core 不依赖 UIKit；
- [ ] UIKit 不依赖具体 Runtime；
- [ ] 无 ChatLayout 或其他第三方聊天布局强制依赖；
- [ ] Layout 实现不泄漏到 public API；
- [ ] 一 Turn 一 Section；
- [ ] 一 Block 一 Item；
...
- 移除 `ChatLayout` 生产依赖；
- 默认布局改为 `UICollectionViewCompositionalLayout`；
- 新增 `AgentUpdateScheduler`、`AgentScrollCoordinator`、`AgentLayoutAnchor` 与 `AgentItemSizeCache`；
- 将 UI Patch 明确分为 `structural`、`sizeAffecting`、`contentOnly`；
- 增加历史 prepend、流式高度变化、键盘、旋转和 iPad resize 的锚点恢复规范；
- 增加滚动漂移、长会话和高频流式的验收指标；
- Phase 3 与 v1.0 验收清单同步更新。
