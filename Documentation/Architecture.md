...
ASTs into a package-owned Sendable render document. `AgentChatUIKit` maps one turn to one collection
section and one block to one item. Renderers emit actions; they never invoke a runtime connection.

The timeline uses a native single-column `UICollectionViewCompositionalLayout`. A package-owned
scroll controller anchors by block ID and relative offset across history prepends, streaming height
changes, keyboard changes, and size transitions. No third-party layout type is exposed or required.
